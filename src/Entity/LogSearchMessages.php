 ...
 */
#[ORM\Entity]
#[ORM\Table(name: 'log_search_messages')]
class LogSearchMessages {
  use BaseEntity;
  /**
   * id
   */
  ...
  protected function buildRow(LogSearchMessages $row) {
    return [
  ...
  protected function insertOrUpdate($id_column, $id_value, string $table, array $values, \Habeuk\MigrateSmf1xTo2x\Configs\QueryBuilderHelper $cQB) {
    ...
    $cQB->from($table, 'dd')->where("$id_column = :id and id_msg = :id_msg ")->setParameter('id', $id_value)->setParameter("id_msg", $values["id_msg"]);
    $query = $cQB->executeQuery();
    if ($query->fetchAssociative()) {
      $cQB->update($table);
      $cQB->setUpdateValues($values);
      $cQB->where("$id_column = :$id_column and id_msg = :id_msg ");
      $cQB->executeQuery();
      $this->results['update']++;
    }
    else {
